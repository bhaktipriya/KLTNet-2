...
def affine_grid_generator(height, width, theta):
    ...
    x = tf.linspace(0.0, tf.cast(width-1,tf.float32), width)
    y = tf.linspace(0.0, tf.cast(height-1,tf.float32), height)
...
def affine_grid_generator_img(H, W, theta):
    batch_grids = affine_grid_generator(H, W, theta)
        
    x_s = (batch_grids[:, 0, :, :] + 1)*0.5*W       
    y_s = (batch_grids[:, 1, :, :] + 1)*0.5*H

    return tf.convert_to_tensor( [x_s, y_s], tf.float32 )
